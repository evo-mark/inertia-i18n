...
export const parseAll = (folderPath, outputPath) => {
	...
	for (const folder of folders) {
		...
		readdirSync(folderPath + sep + folder)
			.filter((file) => !statSync(folderPath + sep + folder + sep + file).isDirectory())
			.sort()
			.forEach((file) => {
				lang[file.replace(/\.\w+$/, "")] = parse(
					readFileSync(folderPath + sep + folder + sep + file).toString(),
				);
			});
	...
	console.log(`Outputting language files in ${data.length} localisation${data.length !== 1 ? "s" : ""}`);
